...
![image-20221029105925228](media/image-20221029105925228.png)
...
Він не потребує реалізації для використання всіх цих механізмів, а також не визначає методів для вимірювання досягнутої якості обслуговування або критеріїв для прийняття рішення про те, коли звільнити транспортне сполучення після погіршення якості обслуговування.
...
For the purpose of this International Standard, the following definitions apply:
 ...
 3.2.4 local matter: A decision made by a system concerning its behavior in the Transport Layer that is not subject to the requirements of this protocol.
...
![image-20221029110056539](media/image-20221029110056539.png)
...
ПРИМІТКА. Цей міжнародний стандарт не включає наступні функції, які розглядаються для включення в наступні видання цього стандарту:
   ...
   c) обмін статусами та моніторинг QOS;
...
f) expedited data (see 6.11), a function used to bypass the flow control of normal data TPDU. Expedited data TPDU flow is controlled by separate flow control; прискорені дані (див. 6.11), функція, яка використовується для обходу керування потоком звичайних даних TPDU. Потік прискорених даних TPDU контролюється окремим контролем потоку;
...
Використання класів і options обговорюється під час встановлення з’єднання. Вибір, який зроблять транспортні суб’єкти, залежатиме від:
   ...
   a) вимоги TS-users, виражені через примітиви служби T-CONNECT;
...
![image-20221029110215311](media/image-20221029110215311.png)  
...
Процедура використовується в усіх класах для призначення транспортних з’єднань мережевим з’єднанням.
...
![image-20221029110530254](media/image-20221029110530254.png)
...
![image-20221029110608984](media/image-20221029110608984.png)
...
![image-20221029110703107](media/image-20221029110703107.png)

 Table 7. (First of 2 pages) Time Parameters related to class 4

![image-20221029110726482](media/image-20221029110726482.png)
...
Таблиця 8 визначає ті TPDU, які дійсні для кожного класу, і код для кожного TPDU.

KEY:  

- xxxx (bits 4-1):  used to signal the CDT (set to 0000 in classes 0 and 1)
...
ПРИМІТКА. Ці коди вже використовуються у відповідних протоколах, визначених організаціями зі стандартизації, відмінними від CCITT/ISO.
...
Усі блоки даних транспортного протоколу (TPDU) повинні містити цілу кількість октетів. Октети в TPDU нумеруються, починаючи з 1 і зростаючи в порядку їх розміщення в NSDU. Біти в октеті пронумеровані від 1 до 8, де біт 1 є молодшим. Коли послідовні октети використовуються для представлення двійкового числа, молодший номер октету має найменше значення.
...
ПРИМІТКА. Коли кодування TPDU представлено за допомогою діаграми в цьому пункті, використовується таке подання:

a) октети показані з октетом з найменшим номером ліворуч, октети з більшим номером розташовані далі праворуч;

b) усередині октету біти відображаються з бітом 8 ліворуч і бітом 1 праворуч.
...
TPDU повинні містити в такому порядку:

   а) `header` - заголовок, що містить:

   1) поле індикатора довжини (`LI`);

   2) `fixed part` - фіксована частина;

   3) `variable part` - змінна частина, якщо вона є;

   b) `data field` - поле даних, якщо воно є.

This structure is illustrated below:

![image-20221029110844046](media/image-20221029110844046.png)
...
Це поле міститься в першому октеті TPDU. Довжина позначається двійковим числом із максимальним значенням 254 (`1111 1110`). Вказана довжина має бути довжиною заголовка в октетах, включаючи параметри, але за винятком поля індикатора довжини та даних користувача, якщо такі є. Значення 255 (`1111 1111`) зарезервовано для можливих розширень. Якщо вказана довжина перевищує розмір наявних даних користувача NS, це є помилкою протоколу.
...
Фіксована частина містить параметри, які часто зустрічаються, включаючи код TPDU. Довжина та структура фіксованої частини визначаються кодом TPDU і в деяких випадках класом протоколу та використовуваними форматами (звичайним або розширеним). Якщо будь-який із параметрів фіксованої частини має недійсне значення або якщо фіксована частина не може міститися в заголовку (як визначено LI), це є помилкою протоколу.
...
ПРИМІТКА. Загалом код TPDU однозначно визначає фіксовану частину. Однак для одного коду TPDU можуть існувати різні варіанти (див. звичайний і розширений формати).
...
Це поле містить код TPDU і міститься в октеті 2 заголовка. Він використовується для означення структури заголовка, що залишився. Це поле є повним октетом, за винятком таких випадків:
...
Змінна частина використовується для визначення менш часто використовуваних параметрів. Якщо присутня змінна частина, вона повинна містити один або більше параметрів.
...
ПРИМІТКА. Кількість параметрів, які можуть міститися у змінній частині, позначається довжиною змінної частини, яка дорівнює `LI` мінус довжина фіксованої частини.

Each parameter contained within the variable part  is  structured as follows:

Кожен параметр, що міститься в змінній частині, має таку структуру:
...
- Поле коду параметра закодовано в двійковій системі; ПРИМІТКА. Без розширень він надає максимум 255 різних параметрів. Однак, як зазначено нижче, біти 8 і 7 не можуть приймати всі можливі значення, тому практична максимальна кількість різних параметрів менша. Код параметра `1111 1111` зарезервовано для можливих розширень коду параметра.
...
- Індикація довжини параметра вказує на довжину поля значення параметра в октетах. ПРИМІТКА. Довжина позначається двійковим числом `m` з теоретичним максимальним значенням 255. Практичне максимальне значення `m` нижче. Наприклад, у випадку одного параметра, що міститься в змінній частині, потрібні два октети для коду параметра та самої індикації довжини параметра. Таким чином, значення `m` обмежене 248. Для більших фіксованих частин заголовка та для кожного наступного параметра максимальне значення m зменшується.
...
- Поле значення параметра містить значення параметра, визначеного в полі коду параметра.

- Ніякі коди параметрів не використовують біти 8 і 7 зі значенням 00.

- Параметри, визначені у змінній частині, можуть бути в будь-якому порядку. Якщо будь-який параметр дублюється, слід використовувати пізніші значення. Параметр, не визначений у цьому міжнародному стандарті, слід розглядати як помилку протоколу в будь-якому отриманому TPDU, крім CR TPDU; у CR TPDU воно має ігноруватися. Якщо транспортний об’єкт-відповідач вибирає клас, для якого параметр CR TPDU не визначено, він може ігнорувати цей параметр, за винятком параметрів класу та опції та альтернативного класу протоколу, які завжди повинні інтерпретуватися. Параметр, визначений у цьому міжнародному стандарті, але має недійсне значення, слід розглядати як помилку протоколу в будь-якому отриманому TPDU, крім CR TPDU. У CR TPDU це має розглядатися як помилка протоколу, якщо це або параметр класу та опції, або параметр альтернативного класу, або параметр додаткової опції; в іншому випадку це буде або проігноровано, або розглядатиметься як помилка протоколу.
...
d)  SRC-REF  :  Reference selected  by  the  transport  entity initiating  the   CR  TPDU  to  identify  the
...
Second  and subsequent choices are listed in the variable part if required.

Bits 4-1 of octet 7 define options to be used  on  the  requested transport connection as follows:

![image-20221029111347234](media/image-20221029111347234.png)
...
![image-20221029111411249](media/image-20221029111411249.png)
...
Дані користувача не дозволені в класі 0 і є необов’язковими в інших класах. Там, де це дозволено, він не може перевищувати 32 октети.
...
Фіксована частина повинна містити:

   a) `CC`: Код підтвердження підключення: `1101`. Біти 8-5 октету 2;

   b) `CDT`: початковий розподіл кредитів (встановлюється на 0000 у класах 0 і 1). Біти 4-1 октету 2;

   c) `DST-REF`: Посилання, що ідентифікує запитане транспортне з’єднання у віддаленому транспортному об’єкті;

   d) `SRC-REF`: Посилання, що ідентифікує запитане транспортне з’єднання у віддаленому транспортному об’єкті.

   e) Class and Option: визначає вибраний клас транспортного протоколу та опцію, яка буде працювати через прийняте транспортне з’єднання відповідно до правил узгодження, визначених у 6.5;
...
Параметри визначені в 13.3.4 і підпадають під дію обмежень у 6.5 (встановлення з’єднання). Параметри, виключені вибором альтернативного класу та опції, не повинні бути присутніми.
...
Дані користувача не дозволені в класі 0, а в інших класах є необов’язковими. Там, де це дозволено, він не може перевищувати 32 октети. Дані користувача підлягають обмеженням правил переговорів (див. 6.5).
...
![image-20221029111500885](media/image-20221029111500885.png)
...
![image-20221029111517745](media/image-20221029111517745.png)
...
Залежно від класу та опції DT TPDU має одну з наступних структур.
...
![image-20221029111553943](media/image-20221029111553943.png)

  c)  Extended Format for use in Classes  2,  3  and  4  when selected during connection establishment.

![image-20221029111602644](media/image-20221029111602644.png)
...
The fixed part shall contain:
  ...
  c)  `EOT`: When set to ONE, indicates that the  current  DT TPDU is the last data unit of a complete DT TPDU
sequence (End of TSDU).  EOT is bit 8 of octet 3 in  class  0  and 1, bit 8 of octet 5 for normal formats for classes 2, 3 and  4  and  bit  8  of octet 8 for extended formats;
...
Стаціонарна частина повинна містити:
   ...
   a) `DT`: Код передачі даних: `1111 0000`;

   b) `DST-REF`: див. 13.4.3;

   c) `EOT`: якщо встановлено значення ONE, вказує, що поточний DT TPDU є останнім блоком даних повного DT TPDU послідовності (End of TSDU). EOT — це біт 8 октету 3 у класі 0 і 1, біт 8 октету 5 для звичайних форматів для класів 2, 3 і 4 і біт 8 октету 8 для розширених форматів;

   d) `TPDU-NR`: TPDU надсилає порядковий номер (нуль у класі 0). Може приймати будь-яке значення в класі 2 без явного керування потоком. `TPDU-NR` — це біти 7-1 октету 3 для класів 0 і 1, біти 7-1 октету 5 для звичайних форматів у класах 2, 3 і 4, октети 5, 6 і 7 разом із бітами 7-1 октету 8 для розширених форматів. ПРИМІТКА. Залежно від класу фіксована частина DT TPDU використовує такі октети:

   Класи 0 і 1: Октети 2-3;
   Класи 2,3,4 звичайний формат: октети від 2 до 5;
   Розширений формат класів 2,3,4: октети 2-8.
...
Це поле містить дані TSDU, що передається. ПРИМІТКА. Довжина цього поля обмежена узгодженим розміром TPDU для цього транспортного з’єднання мінус 3 октети в класах 0 і 1 і мінус 5 октетів (звичайний формат заголовка) або 8 октетів (розширений формат заголовка) в інших класах. Змінна частина, якщо вона присутня, може додатково зменшити розмір поля даних користувача.
...
ED TPDU не повинен використовуватися в класі 0 або в класі 2, якщо не вибрано опцію явного керування потоком або якщо для з’єднання не вибрано службу прискореної передачі даних.

#### 13.8.1  Structure

Depending on the format negotiated  at  connection  establishment the ED TPDU shall have one of the following structures:

Залежно від формату, узгодженого під час встановлення з’єднання, ED TPDU має одну з таких структур:


  a)  Normal Format (classes 1, 2, 3, 4)

![image-20221029111639253](media/image-20221029111639253.png)


  b)  Extended Format (for use in classes 2, 3, 4 when  selected  during connection establishment).

![image-20221029111658710](media/image-20221029111658710.png)
...
The fixed part shall contain:
  ...
  c)  ED-TPDU-NR:  Expedited TPDU  identification  number.   ED-TPDU-NR is used in classes 1, 3 and 4 and may take any value in Class 2.  Bits 7-1 of octet 5  for  normal  formats and octets 5, 6 and 7 together  with  bits  7-1  of  octet  8   for extended formats;

  d)  EOT:   end of TSDU always set to 1 (bit 8 of octet 5 for  normal  formats and bit 8 of octet 8 for
  extended formats).
...
![image-20221029111728367](media/image-20221029111728367.png)

  b)  Extended Format (for use in classes 2, 3, 4 when  selected  during connection establishment).

![image-20221029111746193](media/image-20221029111746193.png)
...
![image-20221029111811514](media/image-20221029111811514.png)

  b)  Extended Format (for use in classes 2, 3,  4  if  selected
 during connection establishment)

![image-20221029111829838](media/image-20221029111829838.png)
...
![image-20221029111852161](media/image-20221029111852161.png)

  b)  Extended Format (for use in classes 3 if  selected  during
 connection establishment).

![image-20221029111909103](media/image-20221029111909103.png)
...
![image-20221029111928813](media/image-20221029111928813.png)
...
![image-20221029112006509](media/image-20221029112006509.png)

NA indicates the procedure is not applicable.
  Table 9. (First of 2 pages) Provision of options

![image-20221029112026120](media/image-20221029112026120.png)
